...
public class ItemController {
    ...
    @GetMapping("/home")
    public String home(){
        return "Welcome to the P1!";
    }

    @GetMapping("{PiD}")
    public ResponseEntity<Item> getItem(@PathVariable("PiD")int itemId){
    ...
    /*
        only admins should be able to get the items searching through database
     */
    @Authorized(allowedRoles = {Role.ADMIN})
    @GetMapping("byname/{productName}")
    public ResponseEntity<Item> getItem(@PathVariable("itemName")String itemName){
    ...
    /*
        only admins should be able to search through items filtered by the price
     */
    @Authorized(allowedRoles = {Role.ADMIN})
    ...
    /*
        only admins should be able to filter through the items by quantity on hand
     */
    @Authorized(allowedRoles = {Role.ADMIN})
    @GetMapping("lowinv/{qoh}")
    public ResponseEntity<Item> getlowItem(@PathVariable("qoh")int qoh){
        authorizationService.guardByUserId(user.getId());
        ResponseEntity responseEntity = null;
        List<Item> item1 = new ArrayList<>();
    ...
    /*
        only admins should be able to delete an item from the database
     */
    @Authorized(allowedRoles = {Role.ADMIN})
    @DeleteMapping("/delete/{PiD}")                 //DELETE - cannot call from browser
    public ResponseEntity<String> deleteItem(@PathVariable("PiD")int PiD){
        authorizationService.guardByUserId(user.getId());
    ...
    /*
        only admins should be able to add new items into the database
     */
    @Authorized(allowedRoles = {Role.ADMIN})
    ...
    /*
        only admins should be able to update a product/item
     */
    @Authorized(allowedRoles = {Role.ADMIN})
    @PutMapping("{PiD}")                 //PUT
